...

{
 ...
 for (int iSample = 0; iSample < 6; iSample++){
   ...
   //HwwZqq input files 
   string inFile("HWWqqqqZqqDataAndMC/Signal/NewSignal/HwwSignalWithB/PileUpBScale/PileUpBScaleOUT");
   string outName("HwwZqq");
   // HwwWqq input files
   if (iSample == 1 )  {
	inFile =  string("HWWqqqqZqqDataAndMC/Signal/NewSignal/HwwSignalWithB/PileUpBScale/HwwWqqPileUpBScaleOUT");
	outName = string("HwwWqq");
   }
   //HbbZqq pass Hww tagger
   if (iSample == 2 ) {
	 inFile = string("HWWqqqqZqqDataAndMC/Signal/NewSignal/HbbSignalPassHww/PileUpBScale/PileUpBScaleOUT");
	 outName = string("HbbZqqHww");
   }
   //HbbWqq pass Hww tagger
   if (iSample == 3) {
	inFile = string("HWWqqqqZqqDataAndMC/Signal/NewSignal/HbbSignalPassHww/PileUpBScale/HbbWqqPileUpBScaleOUT");
	outName = string("HbbWqqHww");
    }
   //HbbZqq pass Hbb
   if (iSample == 4) {
 	inFile = string("HbbZqq/Signal/PlotsGeneration/PileUpBScale/PileUpBScaleOUT");
	outName = string("HbbZqq");
    }
   //HbbWqq pass Hbb
   if (iSample == 5) {
	inFile = string("HbbZqq/Signal/PlotsGeneration/PileUpBScale/HbbWqqPileUpBScaleOUT");
	outName = string("HbbWqq");
    }
   for (int iMass = 0; iMass<33; iMass++){

     //string sInFile = "~hinzmann/public/forMaxime/" + inFile + "" + Form("OUT%d.root", 1000+iMass*100);
     string sInFile = pathWay + inFile + "" + Form("%dGeV.root", 1000+iMass*50);
     cout << sInFile.c_str() << endl;
     TFile file0(sInFile.c_str(), "read");


     string sOutFile = "MiniTrees/Signal_HV/" + outName + Form("OUT%d_miniTree.root", 1000+iMass*50);
     ...
     if ( iSample < 4 ) {
	   ...
	   TH1D* hMass = (TH1D*) file0.Get( "DijetMassHighPuri;1" );
    ...
    if ( iSample > 3 ) {
       for (int iCat = 0; iCat < 5; iCat++){
           TH1D* hMass = (TH1D*) file0.Get( "DijetMassHighP;1" );
