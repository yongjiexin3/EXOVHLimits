...
def Plot(files, label, obs):
	...
	if "Z" in label :
	    efficiencies[mass]=0.03 # assume 10/fb signal cross section
	if "V" in label :
            efficiencies[mass]=0.03 #assume 10/fb signal cross section,
...
#  channels=["HbbZqqHPLP", "HbbWqqHPLP", "HwwZqqHPLPHV", "HwwWqqHPLPHV", "HbbWqqHwwHPLPHV", "HbbZqqHwwHPLPHV", "HWqq", "HZqq"]
    if "Z" in label :
      mg.GetYaxis().SetTitle("#sigma #times BR(Z' #rightarrow HZ) (pb)" )
    if "V" in label :
      mg.GetYaxis().SetTitle("#sigma_{total} #times BR(V'#rightarrow HV) (pb)" )
    ...
    if 'Z' in label :
        ...
        ltheory = " Z'->HZ, HVT(B) " 
    if 'V' in label :
        ftheory=open("../../HVcrossSection.txt")
        #BR = 0.676 
        ltheory = " V'->HV, HVT(B)" 
...
#  channels=["HbbZqqHPLP", "HbbWqqHPLP", "HwwZqqHPLPHV", "HwwWqqHPLPHV", "HbbWqqHwwHPLPHV", "HbbZqqHwwHPLPHV", "HWqq", "HZqq"]
    ...
    if withAcceptance:
        ...
        c1.SaveAs("brazilianFlagHV_%s.pdf" %label)
        c1.SaveAs("brazilianFlagHV_%s.eps" %label)
