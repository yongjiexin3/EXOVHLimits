...
double kZ = 1.0;
double kW = 2.0; // the fraction of W and Z cross section as of total cross section
...
//    RooExtendPdf bkg_fitTmp(TString::Format("bkg_fit_%s",cat_names.at(c).c_str()),"",*bkg_fitTmp0,*w->var(TString::Format("bkg_fit_norm_%s",cat_names.at(c).c_str())));
   //RooExtendPdf bkg_fitTmp(TString::Format("bkg_fit_%s",cat_names.at(c).c_str()),"",*bkg_fitTmp0,*w->var(TString::Format("bkg_fit_%s_norm",cat_names.at(c).c_str()))); 
...
//  }
  ...
  if (signalname=="HwwWqq")   {
        TFile inputFile(TString(pathFile+Form("HZAnalysis/HWWqqqqZqqDataAndMC/Signal/NewSignal/HwwSignalWithB/PileUpBScale/HwwWqqPileUpBScaleOUT%.0fGeV.root", mass))); // HwwWqq
        if(iChan==0) 
        ...
        TFile inputFile2(TString(pathFile+Form("HZAnalysis/HWWqqqqZqqDataAndMC/Signal/NewSignal/HbbSignalPassHww/PileUpBScale/HbbWqqPileUpBScaleOUT%.0fGeV.root", mass))); // HbbWqq
        if(iChan==0)
    	...
    	TFile inputFile3(TString(pathFile+Form("HZAnalysis/HWWqqqqZqqDataAndMC/Signal/NewSignal/HwwSignalWithB/PileUpBScale/PileUpBScaleOUT%.0fGeV.root", mass)));  // HwwZqq pass Hww tagger
        if(iChan==0)
            hDijetMass3 = (TH1F*) inputFile3.Get("DijetMassHighPuri;1");
        else if(iChan==1)
            hDijetMass3 = (TH1F*) inputFile3.Get("DijetMassLowPuriV;1");
        else if(iChan==2)
            hDijetMass3 = (TH1F*) inputFile3.Get("DijetMassLowPuriH;1");
        TFile inputFile4(TString(pathFile+Form("HZAnalysis/HWWqqqqZqqDataAndMC/Signal/NewSignal/HbbSignalPassHww/PileUpBScale/PileUpBScaleOUT%.0fGeV.root", mass))); // HbbZqq pass Hww tagger
        if(iChan==0)
            hDijetMass4 = (TH1F*) inputFile4.Get("DijetMassHighPuri;1");
        else if(iChan==1)
            hDijetMass4 = (TH1F*) inputFile4.Get("DijetMassLowPuriV;1");
        else if(iChan==2)
            hDijetMass4 = (TH1F*) inputFile4.Get("DijetMassLowPuriH;1");

        outFile <<  "rate                      "
          << hDijetMass3->Integral()*scaleFactor*BRHqqqq*BRZ*kZ  << "  " << hDijetMass->Integral()*scaleFactor*BRHqqqq*BRW*kW  << "  "<< hDijetMass4->Integral()*scaleFactor*BRHbb*BRZ*kZ  <<  "  " << hDijetMass2->Integral()*scaleFactor*BRHbb*20000/numbers[int((mass-1000)/100.0)]*BRW*kW << "  " << 1 << endl;
  } //Finish with HbbWqq
  ...
  if (signalname=="HwwZqq")  {
        ...
        if(iChan==0)
            ...
            hDijetMass2 = (TH1F*) inputFile2.Get("DijetMassLowPuriH;1");
        outFile <<  "rate                      "
          << hDijetMass->Integral()*scaleFactor*BRHqqqq*BRZ  << "  0 " << hDijetMass2->Integral()*scaleFactor*BRHbb*BRZ << "  0  " << 1*(BRHbb+BRHqqqq) << endl;
  ...
  outFile << "CMS_crossTalk      lnN  1.31  1.31  1.31  1.31     - # cross talk from other signals" << endl;
  ...
  if(signalname=="HbbWqq") {
        TFile inputFile(TString(pathFile+Form("HZAnalysis/HbbZqq/Signal/PlotsGeneration/PileUpBScale/HbbWqqPileUpBScaleOUT%.0fGeV.root", mass))); // HbbWqq
        if(iChan==3)
            hDijetMass = (TH1F*) inputFile.Get("DijetMassHighP;1");
        else if(iChan==4)
            hDijetMass = (TH1F*) inputFile.Get("DijetMassLowP;1");
        TFile inputFile2(TString(pathFile+Form("HZAnalysis/HbbZqq/Signal/PlotsGeneration/PileUpBScale/PileUpBScaleOUT%.0fGeV.root", mass)));
        if(iChan==3)  //HbbZqq
            hDijetMass2 = (TH1F*) inputFile2.Get("DijetMassHighP;1");
        else if(iChan==4) //HbbZqq
            hDijetMass2 = (TH1F*) inputFile2.Get("DijetMassLowP;1");

        outFile <<  "rate                      "
          << hDijetMass2->Integral()*scaleFactor*BRHbb*BRZ*kZ  <<"  " << hDijetMass->Integral()*scaleFactor*BRHbb*20000/numbers[int((mass-1000)/100)]*BRW*kW << "  " << 1 << endl;
  }
  if(signalname=="HbbZqq") {
        ...
        if(iChan==3)
            ...
            hDijetMass = (TH1F*) inputFile.Get("DijetMassLowP;1");
        outFile <<  "rate                      "
          << " " << hDijetMass->Integral()*scaleFactor*BRHbb << " 0  " << 1*BRHbb << endl;
  ...
  if(iChan==3){
      outFile << "CMS_eff_tau21_sf         lnN  1.076  1.076      - # tau21 efficiency" << endl;
...
//  outFile << Form("CMS_eff_vtag_mass_sf_%s          lnN  1.093  1.099      - # jet mass efficiency",cat_names[iChan].c_str()) << endl;
  ...
  // anti-correlated the high purity (1.076) and low purity (0.54) categories
      outFile << "CMS_eff_tau21_sf         lnN  0.54  0.54      - # tau21 efficiency" << endl;
